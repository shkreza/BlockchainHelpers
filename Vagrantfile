...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "node", type: "shell", inline: <<-SHELL
    ...
    sudo ln -f -s `pwd`/node-v8.9.4-linux-x64/bin/npx /bin/
    sudo ln -f -s `pwd`/node-v8.9.4-linux-x64/bin/yo-complete /bin/
    sudo ln -f -s `pwd`/node-v8.9.4-linux-x64/bin/yo /bin/
    sudo chown vagrant -R /home/vagrant
  SHELL

  config.vm.provision "composer", type: "shell", inline: <<-SHELL
